...
public class ComplexHeatMapEnrichment implements Task{
     ...
     */
    public ComplexHeatMapEnrichment (ArrayList<Complex> complexesSelected, Map<String, ComplexEdge> complexEdges, boolean isPositive){
    ...
    protected void fillScoreArray(){
       ...
       for (int row = 0; row < nQuery; row++){
            ...
            for (int col = 0; col < nArray; col++){
                ...
                //Same complex case
                if(comp1 == comp2){
                    System.out.println("SAME COMPLEX");
                    Double posPval = comp1.posPValue();
                    if(isPositive){
                        if(posPval != null)
                            pclRow.append(-Math.log10(posPval)+"\t");
                        else
                            pclRow.append(" \t");
                    }
                    Double negPval = comp1.negPValue();
                    if(!isPositive){
                        if(negPval != null)
                            pclRow.append(-Math.log10(negPval)+"\t");
                        else
                            pclRow.append(" \t");
                    }
                ...
                if (ce == null){
                    ...
                    if (significance == null){
                        ...
                        || significance.equals(Config.edgeNegSignificanceKey)){
                            ...
                            if(isPositive){
                                if(ce.posRelations() != 0)
                                    pclRow.append(-Math.log10(ce.posPValue())+"\t");
                                else
                                    pclRow.append(" \t");
                            }
                            if(!isPositive){
                                if(ce.negRelations() != 0)
                                    pclRow.append(-Math.log10(ce.negPValue())+"\t");
                                else
                                    pclRow.append(" \t");
                            }
                        }
